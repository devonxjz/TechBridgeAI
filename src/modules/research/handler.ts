// ═══════════════════════════════════════════════════════
// Framework-neutral Research Handler (SSE Streaming)
// Cache-first: checks research cache before initializing expensive paid
// providers (Search, Scraper, LLM, Registry).
// ═══════════════════════════════════════════════════════
...
import type { StorageAdapter, StorageContext } from "@/adapters/storage/types";
import {
  createResearchCache,
...
export interface TrustedResearchContext {
  tenantId: string;
  userId: string;
  requestId: string;
}
...
export async function handleResearchRequest(
  req: Request,
  context: TrustedResearchContext,
): Promise<Response> {
  ...
  try {
    ...
    if (action === "auto") {
      autoResolution = await researchCache.lookup(storageContext, input, {
        signal: req.signal,
      });
      ...
      selectedSnapshot = await researchCache.select(
        storageContext,
        input,
        selectedCompanyId,
        { signal: req.signal },
    ...
    const publicError = toPublicResearchError(err);
    return new Response(
      JSON.stringify({ error: publicError.message, code: publicError.code }),
      { status: 503, headers: { "Content-Type": "application/json" } }
    );
  }
    ...
    try {
      if (action === "auto" && autoResolution) {
        ...
        if (autoResolution.kind === "miss") {
          ...
          const miss = await researchCache.resolveMiss(storageContext, input, {
      ...
      writer.write({
        event: "error",
        data: { message: publicError.message, code: publicError.code },
      } as StreamEvent);
      writer.write({ event: "done", data: {} } as StreamEvent);
    } finally {
